    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-4">
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/30 text-purple-400 text-xs font-mono"
        >
        ...
        <p className="text-gray-300 text-base sm:text-lg">
          Explore my open-source work, full-stack applications, mobile apps, and AI document extraction tools.
        </p>
      </div>
          ...
          return (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-mono font-medium transition-all ${
                isActive
                  ? 'bg-gradient-to-r from-cyan-500 to-purple-600 text-slate-950 shadow-lg shadow-cyan-500/20 font-bold scale-105'
        ...
        <AnimatePresence mode="popLayout">
          {filteredProjects.map((project) => (
            <motion.div
              ...
              className="glass-panel p-6 rounded-3xl border border-white/10 flex flex-col justify-between hover:border-cyan-400/50 hover:shadow-[0_0_30px_rgba(0,245,255,0.15)] transition-all group"
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-3">
                  <span className="text-[10px] font-mono uppercase px-2.5 py-1 rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 font-semibold">
                    {project.category}
                  </span>
                  {project.featured && (
              ...
              <div className="mt-6 pt-4 border-t border-white/10 space-y-4">
                <div className="flex flex-wrap gap-1.5">
                  {project.tech_stack.map((tech) => (
                    <span
                      key={tech}
                      className="text-[11px] font-mono px-2 py-0.5 rounded-md bg-slate-900 text-gray-300 border border-white/5"
                    >
                      {tech}
                    </span>
                  ))}
        ...
        {selectedProject && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            <motion.div
              ...
              className="w-full max-w-2xl bg-slate-950 border border-cyan-500/30 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 relative overflow-hidden"
            >
              <button
                onClick={() => setSelectedProject(null)}
                className="absolute top-5 right-5 p-2 rounded-xl text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>

              <div>
                <span className="text-xs font-mono uppercase px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/30">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedProject.tech_stack.map((t) => (
                    <span key={t} className="text-xs font-mono px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-cyan-300">
                      {t}
                    </span>
                  ))}
                </div>
              ...
              <div className="pt-4 border-t border-white/10 flex items-center justify-end gap-4">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-6 py-3 rounded-full font-bold text-slate-950 bg-gradient-to-r from-cyan-400 to-purple-400 hover:opacity-90 flex items-center gap-2 text-xs uppercase tracking-wider"
                >
                  <GithubIcon className="w-4 h-4" />
